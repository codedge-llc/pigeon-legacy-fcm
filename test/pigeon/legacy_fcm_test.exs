defmodule Pigeon.LegacyFCMTest do
  ...
  describe "handle_push/3" do
    test "successfully sends a valid push" do
      ...
      assert notification.response in [:unknown, [success: valid_fcm_reg_id()]]
    end
    ...
    test "successfully handles multiple registration_ids" do
      ...
      assert notification.response[:success] in [nil, valid_fcm_reg_id()]
      assert notification.response[:invalid_registration] in [nil, "bad_reg_id"]
    end
    ...
    test "successfully sends a valid push with callback" do
      ...
      assert n.status in [:success, nil]
      assert regids in [:unknown, [success: valid_fcm_reg_id()]]
    end
    ...
    test "returns an error on pushing with a bad registration_id" do
      ...
      assert n.status in [nil, :success]
      assert n.response in [:unknown, [invalid_registration: reg_id]]
